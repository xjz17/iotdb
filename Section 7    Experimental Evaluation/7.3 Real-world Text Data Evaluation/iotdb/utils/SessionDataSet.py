...
class SessionDataSet(object):
    ...
    def construct_row_record_from_value_array(self):
        ...
        return RowRecord(
            struct.unpack(">q", self.iotdb_rpc_data_set.get_time_bytes())[
                0], out_fields
        )
...
def get_typed_point(field: Field, none_value=None):
    ...
    if result_next_type == TSDataType.BOOLEAN:
        if field.get_bool_value():
            return 1
        else:
            return 0
    elif result_next_type == TSDataType.DOUBLE:
        return field.get_double_value()
    elif result_next_type == TSDataType.FLOAT:
        return field.get_float_value()
    elif result_next_type == TSDataType.TEXT:
        return field.get_string_value()
    elif result_next_type == TSDataType.INT64:
        return field.get_long_value()
    elif result_next_type == TSDataType.INT32:
