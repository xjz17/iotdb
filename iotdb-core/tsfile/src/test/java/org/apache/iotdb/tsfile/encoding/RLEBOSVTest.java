...
public class RLEBOSVTest {
    ...
    private static int BOSBlockEncoder(int[] ts_block, int block_i, int block_size, int remaining, int encode_pos,
        ...
        encode_pos = BOSEncodeBits(ts_block_delta, init_block_size, final_k_start_value, final_x_l_plus,
                final_k_end_value, final_x_u_minus,
    ...
    private static int BOSEncodeBitsImprove(int[] ts_block_delta,
        ...
        for (int i = 0; i < block_size; i++) {
            ...
            if (cur_value <= final_k_start_value) {
                ...
                // encode_pos = EncodeBits(cur_value-
                // final_k_end_value,right_bit_width,encode_pos,cur_byte);
                // final_right_outlier.add(cur_value - final_k_end_value);
                final_right_outlier_index.add(i);
                if (cur_index_bitmap_outlier_bits % 8 != 7) {
                    index_bitmap_outlier <<= 2;
                ...
                // final_normal.add(cur_value - final_x_l_plus);
                // encode_pos = EncodeBits(cur_value-
                // final_x_l_plus,right_bit_width,encode_pos,cur_byte);
                index_bitmap_outlier <<= 1;
                cur_index_bitmap_outlier_bits += 1;
            }
            if (cur_index_bitmap_outlier_bits % 8 == 0) {
        ...
        // encode_pos =encodeOutlier2Bytes(repeat_count,
        // getBitWith(init_block_size-1),encode_pos,cur_byte);
        ...
        for (int cur_value : ts_block_delta) {
            if (cur_value <= final_k_start_value) {
                ...
                encode_pos = EncodeBits(cur_value - final_x_l_plus, bit_width_final, encode_pos, cur_byte,
                        bit_index_list);
            }
        }
        if (bit_index_list[0] != 8) {
        ...
        // }
        // } else {
        // encode_pos = encodeOutlier2Bytes(final_left_outlier_index,
        // getBitWith(block_size-1),encode_pos,cur_byte);
        // encode_pos = encodeOutlier2Bytes(final_right_outlier_index,
        // getBitWith(block_size-1),encode_pos,cur_byte);
        // }
        // }
        ...
        // bit_width_final,encode_pos,cur_byte);
        // if (k1 != 0)
        // encode_pos = encodeOutlier2Bytes(final_left_outlier,
        // left_bit_width,encode_pos,cur_byte);
        // if (k2 != 0)
        // encode_pos = encodeOutlier2Bytes(final_right_outlier,
        // right_bit_width,encode_pos,cur_byte);
        // System.out.println(encode_pos);
        return encode_pos;
    ...
    private static int BOSBlockEncoderImprove(int[] ts_block, int block_i, int block_size, int remaining,
        ...
        encode_pos = BOSEncodeBitsImprove(ts_block_delta, init_block_size, final_k_start_value, final_x_l_plus,
                final_k_end_value, final_x_u_minus,
                max_delta_value, min_delta, repeat_count, encode_pos, cur_byte);
    ...
    public static int BOSBlockDecoderImprove(byte[] encoded, int decode_pos, int[] value_list, int init_block_size,
        ...
        // ArrayList<Integer> decode_pos_normal = new ArrayList<>();
        // final_normal = decodeOutlier2Bytes(encoded, decode_pos, bit_width_final,
        // block_size - k1 - k2, decode_pos_normal);
        //
        // decode_pos = decode_pos_normal.get(0);
        // if (k1 != 0) {
        // ArrayList<Integer> decode_pos_result_left = new ArrayList<>();
        // final_left_outlier = decodeOutlier2Bytes(encoded, decode_pos, left_bit_width,
        // k1, decode_pos_result_left);
        // decode_pos = decode_pos_result_left.get(0);
        // }
        // if (k2 != 0) {
        // ArrayList<Integer> decode_pos_result_right = new ArrayList<>();
        // final_right_outlier = decodeOutlier2Bytes(encoded, decode_pos,
        // right_bit_width, k2, decode_pos_result_right);
        // decode_pos = decode_pos_result_right.get(0);
        // }
        int left_outlier_i = 0;
        int right_outlier_i = 0;
        ...
        int leftOutlierNextIndex = (left_outlier_i < k1) ? final_left_outlier_index.get(left_outlier_i)
                : Integer.MAX_VALUE;
        int rightOutlierNextIndex = (right_outlier_i < k2) ? final_right_outlier_index.get(right_outlier_i)
                : Integer.MAX_VALUE;
        decode_list[0] = decode_pos;
        // decode_list[1]= 8;
        ...
        for (int i = 0; i < cur_block_size; i++) {
            ...
            if (i == leftOutlierNextIndex) {
                ...
                left_outlier_i++;
                leftOutlierNextIndex = (left_outlier_i < k1) ? final_left_outlier_index.get(left_outlier_i)
                        : Integer.MAX_VALUE;
                ...
                right_outlier_i++;
                rightOutlierNextIndex = (right_outlier_i < k2) ? final_right_outlier_index.get(right_outlier_i)
                        : Integer.MAX_VALUE;
            } else {
    ...
    public void BOSImproveTimeTest() throws IOException {
        ...
        String output_parent_dir = parent_dir
                + "icde0802/supply_experiment/R1O4_decode_time/compression_ratio/rle_bos_v";
        String input_parent_dir = parent_dir + "trans_data/";
        ArrayList<String> input_path_list = new ArrayList<>();
        ArrayList<String> output_path_list = new ArrayList<>();
        ArrayList<String> dataset_name = new ArrayList<>();
