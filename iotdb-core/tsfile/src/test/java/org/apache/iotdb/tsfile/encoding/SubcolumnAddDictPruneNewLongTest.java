...
public class SubcolumnAddDictPruneNewLongTest {

  private static final int[] DEFAULT_THRESHOLD = {
      2, 3, 5, 8, 9, 11, 14, 16, 17, 17, 18, 19, 20, 21, 22, 22, 23, 24, 24, 24, 25, 25, 26, 26,
      26, 26, 27, 27, 27, 27, 27, 27
  };
  private static final int[] THRESHOLD_64 = {
      2, 3, 5, 9, 13, 17, 19, 24, 29, 32, 33, 33, 35, 37, 39, 40, 42, 43, 44, 45, 46, 47, 48, 48,
      49, 50, 50, 51, 51, 52, 52, 52
  };
  private static final int[] THRESHOLD_128 = {
      2, 3, 5, 9, 17, 22, 33, 33, 43, 52, 59, 64, 65, 65, 69, 72, 76, 79, 81, 84, 86, 88, 90, 91,
      93, 94, 95, 96, 98, 99, 100, 100
  };
  private static final int[] THRESHOLD_256 = {
      2, 3, 5, 9, 17, 33, 37, 64, 65, 77, 94, 107, 119, 128, 129, 129, 136, 143, 149, 154, 159,
      163, 167, 171, 175, 178, 181, 183, 186, 188, 190, 192
  };
  private static final int[] THRESHOLD_512 = {
      2, 3, 5, 9, 17, 33, 65, 65, 114, 129, 140, 171, 197, 220, 239, 256, 257, 257, 270, 282, 293,
      303, 312, 320, 328, 335, 342, 348, 354, 359, 364, 368
  };
  private static final int[] THRESHOLD_1024 = {
      2, 3, 5, 9, 17, 33, 65, 128, 129, 205, 257, 257, 316, 366, 410, 448, 482, 512, 513, 513, 537,
      559, 579, 598, 615, 631, 645, 659, 671, 683, 694, 704
  };
  private static final int[] THRESHOLD_2048 = {
      2, 3, 5, 9, 17, 33, 65, 129, 228, 257, 373, 512, 513, 586, 683, 768, 844, 911, 971, 1024,
      1025, 1025, 1069, 1110, 1147, 1182, 1214, 1244, 1272, 1298, 1322, 1344
  };
  private static final int[] THRESHOLD_4096 = {
      2, 3, 5, 9, 17, 33, 65, 129, 257, 410, 513, 683, 946, 1025, 1093, 1280, 1446, 1593, 1725,
      1844, 1951, 2048, 2049, 2049, 2130, 2206, 2276, 2341, 2402, 2458, 2511, 2560
  };
  private static final int[] THRESHOLD_8192 = {
      2, 3, 5, 9, 17, 33, 65, 129, 257, 513, 745, 1025, 1261, 1756, 2049, 2049, 2410, 2731, 3019,
      3277, 3511, 3724, 3918, 4096, 4097, 4097, 4248, 4389, 4520, 4643, 4757, 4864
  };
  ...
  public static void pack8Values(int[] values, int offset, int width, int encodePos, byte[] encodedResult) {
    int bufIdx = 0;
    int valueIdx = offset;
    int leftBit = 0;
  ...
  public static void unpack8Values(byte[] encoded, int offset, int width, int[] resultList, int resultOffset) {
    int byteIdx = offset;
    long buffer = 0;
    int totalBits = 0;
    int valueIdx = 0;
  ...
  public static int bitPacking(int[] numbers, int bitWidth, int encodePos, byte[] encodedResult, int numValues) {
  ...
  public static int decodeBitPacking(byte[] encoded, int decodePos, int bitWidth, int numValues, int[] resultList) {
    int blockNum = numValues / 8;
    int remainder = numValues % 8;
  ...
  public static void long2Bytes(long integer, int encodePos, byte[] currentBytes) {
    currentBytes[encodePos] = (byte) (integer >> 56);
    currentBytes[encodePos + 1] = (byte) (integer >> 48);
    currentBytes[encodePos + 2] = (byte) (integer >> 40);
    currentBytes[encodePos + 3] = (byte) (integer >> 32);
    currentBytes[encodePos + 4] = (byte) (integer >> 24);
    currentBytes[encodePos + 5] = (byte) (integer >> 16);
    currentBytes[encodePos + 6] = (byte) (integer >> 8);
    currentBytes[encodePos + 7] = (byte) integer;
  }
  ...
  private static int countGroupedRuns(long[] values, int length, int shiftAmount, int mask) {
  ...
  private static int countDistinctValuesUntilLimit(long[] values, int length, int shiftAmount, int mask, int limit) {
    int seenMask = 0;
    int distinctCount = 0;
  ...
  public static int Subcolumn(long[] x, int xLength, int m, int blockSize, int[] encodingType) {
    if (m == 0) {
      return 1;
    }
    ...
    for (int i = 0; i < m; i++) {
      int currentValue = (int) ((x[0] >> i) & 1L);
      boolean hasOne = currentValue == 1;
      int runCount = 1;
      boolean changed = false;

      for (int j = 1; j < xLength; j++) {
        int subcolumnValue = (int) ((x[j] >> i) & 1L);
        if (subcolumnValue == 1) {
          hasOne = true;
        }
    ...
    for (int beta : BETA_LIST) {
      ...
      for (int i = 0; i < l; i++) {
        ...
        if (deCostMax < currentCost) {
          int distinctCount = countDistinctValuesUntilLimit(x, xLength, groupStart, mask, threshold[beta - 1]);
  ...
  public static int SubcolumnEncoder(long[] list, int encodePos, byte[] encodedResult, int[] beta, int blockSize, int[] encodingType) {
    ...
    for (int i = 0; i < l; i++) {
      ...
      for (int j = 0; j < listLength; j++) {
        int current = (int) ((list[j] >> shiftAmount) & mask);
    ...
    for (int i = 0; i < l; i++) {
      int shiftAmount = i * betaValue;
      for (int j = 0; j < listLength; j++) {
        subcolumnBuffer[j] = (int) ((list[j] >> shiftAmount) & mask);
      }
      ...
      if (encodingType[i] == 2) {
        ...
        encodePos = bitPacking(dictKeyList, bitWidthList[i], encodePos, encodedResult, cardinality);
        encodePos = bitPacking(subcolumnBuffer, dictBitWidth, encodePos, encodedResult, listLength);
        continue;
      }

      if (encodingType[i] == 0) {
        encodePos = bitPacking(subcolumnBuffer, bitWidthList[i], encodePos, encodedResult, listLength);
      } else {
        ...
        encodePos = bitPacking(rleValues, bitWidthList[i], encodePos, encodedResult, runCount);
      }
    }
  ...
  public static int SubcolumnDecoder(byte[] encodedResult, int encodePos, long[] list, int blockSize) {
    int listLength = list.length;
    ...
    for (int i = 0; i < l; i++) {
      ...
      if (type == 0) {
        encodePos = decodeBitPacking(encodedResult, encodePos, currentBitWidth, listLength, subcolumnBuffer);
      } else if (type == 1) {
        int index = ((encodedResult[encodePos] & 0xFF) << 8) | (encodedResult[encodePos + 1] & 0xFF);
        encodePos += 2;

        encodePos = decodeBitPacking(encodedResult, encodePos, bw, index, runLength);
        encodePos = decodeBitPacking(encodedResult, encodePos, currentBitWidth, index, rleValues);
        ...
        int cardinality = ((encodedResult[encodePos] & 0xFF) << 8) | (encodedResult[encodePos + 1] & 0xFF);
        encodePos += 2;
        int dictBitWidth = bitWidth(cardinality);
        int[] dictKeyList = new int[cardinality];

        encodePos = decodeBitPacking(encodedResult, encodePos, currentBitWidth, cardinality, dictKeyList);
        encodePos = decodeBitPacking(encodedResult, encodePos, dictBitWidth, listLength, subcolumnBuffer);
      ...
      for (int j = 0; j < listLength; j++) {
        list[j] |= ((long) subcolumnBuffer[j]) << shiftAmount;
      }
    }
  ...
  public static long[] getAbsDeltaTsBlock(long[] tsBlock, int blockIndex, int blockSize, int remaining, long[] minDelta) {
    long[] tsBlockDelta = new long[remaining];
    long valueDeltaMin = Long.MAX_VALUE;
    int base = blockIndex * blockSize;
    int end = base + remaining;

    for (int j = base; j < end; j++) {
      long current = tsBlock[j];
  ...
  public static int BlockEncoder(long[] data, int blockIndex, int blockSize, int remainder, int encodePos, byte[] encodedResult, int[] beta) {
    long[] minDelta = new long[1];
    long[] dataDelta = getAbsDeltaTsBlock(data, blockIndex, blockSize, remainder, minDelta);

    long2Bytes(minDelta[0], encodePos, encodedResult);
    encodePos += 8;
  ...
  public static int BlockDecoder(byte[] encodedResult, int blockIndex, int blockSize, int remainder, int encodePos, long[] data) {
    long minDelta = bytes2Long(encodedResult, encodePos, 8);
    encodePos += 8;

    long[] blockData = new long[remainder];
  ...
  public static int Encoder(long[] data, int blockSize, byte[] encodedResult) {
    ...
    if (remainder <= 3) {
      ...
      for (int i = 0; i < remainder; i++) {
        long2Bytes(data[base + i], encodePos, encodedResult);
        encodePos += 8;
      }
    } else {
  ...
  public static long[] Decoder(byte[] encodedResult) {
    int encodePos = 0;
    ...
    if (remainder <= 3) {
      int base = numBlocks * blockSize;
      for (int i = 0; i < remainder; i++) {
        data[base + i] = bytes2Long(encodedResult, encodePos, 8);
        encodePos += 8;
      }
    } else {
  ...
  public void test0() throws IOException {
    ...
    String outputPath = outputParentDir + "subcolumn_adddict_prunenew_long.csv";
    ...
    for (File file : csvFiles) {
      ...
      long[] data2Arr = new long[data1.size()];
      long maxMul = (long) Math.pow(10, maxDecimal);
      for (int i = 0; i < data1.size(); i++) {
        data2Arr[i] = (long) (data1.get(i) * maxMul);
      }

      byte[] encodedResult = new byte[Math.max(16, data2Arr.length * 40)];
      int length = 0;
