...
//        raw_length.set(4,value_delta_min);
  ...
  public static ArrayList<Byte> encodeRLETimeBitWidth2Bytes(
          ArrayList<ArrayList<Integer>> bit_width_segments) {
    ArrayList<Byte> encoded_result = new ArrayList<>();
  ...
  public static ArrayList<Byte> encodeRLEValueBitWidth2Bytes(
          ArrayList<ArrayList<Integer>> bit_width_segments) {
    ArrayList<Byte> encoded_result = new ArrayList<>();
  ...
  public static ArrayList<ArrayList<Byte>> ReorderingRegressionEncoder(
          ArrayList<ArrayList<Integer>> data, int block_size, int[] third_value, int segment_size, int k) {
    block_size++;
    ArrayList<ArrayList<Byte>> encoded_result = new ArrayList<>();
    ArrayList<Byte> encoded_result_time = new ArrayList<Byte>();
    ArrayList<Byte> encoded_result_value = new ArrayList<Byte>();
    int length_all = data.size();
    byte[] length_all_bytes = int2Bytes(length_all);
    for (byte b : length_all_bytes) encoded_result_time.add(b);
    for (byte b : length_all_bytes) encoded_result_value.add(b);
    int block_num = length_all / block_size;

    // encode block size (Integer)
    byte[] block_size_byte = int2Bytes(block_size);
    for (byte b : block_size_byte) encoded_result_time.add(b);
    for (byte b : block_size_byte) encoded_result_value.add(b);
    ...
    if (length_partition < length_time && length_partition < length_value) { // partition performs better
      ...
      for (int i = 0; i < block_num; i++) {
        ...
        ArrayList<ArrayList<Byte>> cur_encoded_result = encodeSegment2BytesFinal(ts_block_delta, bit_width_segments, raw_length, segment_size, theta, result2);
      ...
      for (int i = 0; i < block_num; i++) {
        ...
        ArrayList<ArrayList<Byte>> cur_encoded_result = encodeSegment2BytesFinal(ts_block_delta, bit_width_segments, raw_length, segment_size, theta, result2);
        encoded_result_time.addAll(cur_encoded_result.get(0));
        encoded_result_value.addAll(cur_encoded_result.get(1));
      }
    }
    ...
    if (remaining_length == 1) {
      ...
      for (byte b : timestamp_end_bytes) encoded_result_time.add(b);
      byte[] value_end_bytes = int2Bytes(data.get(data.size() - 1).get(1));
      for (byte b : value_end_bytes) encoded_result_value.add(b);
    }
    ...
    encoded_result.add(encoded_result_time);
    encoded_result.add(encoded_result_value);
    return encoded_result;
  }
  ...
  private static ArrayList<ArrayList<Byte>> encodeSegment2BytesFinal(ArrayList<ArrayList<Integer>> delta_segments, ArrayList<ArrayList<Integer>> bit_width_segments, ArrayList<Integer> raw_length, int segment_size, ArrayList<Float> theta, ArrayList<Integer> result2) {
    ArrayList<ArrayList<Byte>> encoded_result = new ArrayList<>();
    ArrayList<Byte> encoded_result_time = new ArrayList<>();
    ArrayList<Byte> encoded_result_value = new ArrayList<>();
    int block_size = delta_segments.size();
    int segment_n = block_size / segment_size;

    // encode interval0 and value0
    byte[] interval0_byte = int2Bytes(delta_segments.get(0).get(0));
    for (byte b : interval0_byte) encoded_result_time.add(b);
    byte[] value0_byte = int2Bytes(delta_segments.get(0).get(1));
    for (byte b : value0_byte) encoded_result_value.add(b);

    // encode theta
    byte[] theta0_r_byte = float2bytes(theta.get(0) + raw_length.get(3));
    for (byte b : theta0_r_byte) encoded_result_time.add(b);
    byte[] theta1_r_byte = float2bytes(theta.get(1));
    for (byte b : theta1_r_byte) encoded_result_time.add(b);
    byte[] theta0_v_byte = float2bytes(theta.get(2) + raw_length.get(4));
    for (byte b : theta0_v_byte) encoded_result_value.add(b);
    byte[] theta1_v_byte = float2bytes(theta.get(3));
    for (byte b : theta1_v_byte) encoded_result_value.add(b);
    
    encoded_result_time.addAll(encodeRLETimeBitWidth2Bytes(bit_width_segments));
    encoded_result_value.addAll(encodeRLEValueBitWidth2Bytes(bit_width_segments));
    
    for (int segment_i = 0; segment_i < segment_n; segment_i++) {
      int bit_width_time = bit_width_segments.get(segment_i).get(0);
      int bit_width_value = bit_width_segments.get(segment_i).get(1);
      byte[] timestamp_bytes = bitPacking(delta_segments, 0, segment_i * segment_size + 1, segment_size, bit_width_time);
      for (byte b : timestamp_bytes) encoded_result_time.add(b);
      byte[] value_bytes = bitPacking(delta_segments, 1, segment_i * segment_size + 1, segment_size, bit_width_value);
      for (byte b : value_bytes) encoded_result_value.add(b);
    }

    byte[] td_common_byte = int2Bytes(result2.get(0));
    for (byte b : td_common_byte) encoded_result_time.add(b);
    for (byte b : td_common_byte) encoded_result_value.add(b);
