...
public class ReorderingEncodeRegression32IntLearningRateTestAdjust2H {
  ...
  //  public static void splitTimeStamp(ArrayList<ArrayList<Integer>> ts_block, int block_size, int
  // td,
  //                                    ArrayList<Integer> deviation_list,ArrayList<Integer>
  // result){
  ...
  private static ArrayList<Integer> adjustTo0(
    ...
    for (int i = 1; i < block_size; i++) {
      ...
      if (i == (alpha + 1)) {
        timestamp_delta_i =
            ts_block.get(alpha + 1).get(0) - (theta0_t + theta1_t * ts_block.get(alpha - 1).get(0));
        value_delta_i =
            ts_block.get(alpha + 1).get(1) - (theta0_v - theta1_v * ts_block.get(alpha - 1).get(1));
      } else if (i == alpha) {
        timestamp_delta_i =
            ts_block.get(0).get(0) - (theta0_t + theta1_t * ts_block.get(alpha).get(0));
  ...
  private static ArrayList<Integer> adjustAlphaToJ(
    ...
    for (int i = 1; i < block_size; i++) {
      ...
      if (i == j) {
        timestamp_delta_i =
            ts_block.get(j).get(0) - (theta0_t + theta1_t * ts_block.get(alpha).get(0));
        value_delta_i = ts_block.get(j).get(1) - (theta0_v + theta1_v * ts_block.get(alpha).get(1));
      } else if (i == alpha) {
        timestamp_delta_i =
            ts_block.get(alpha).get(0) - (theta0_t + theta1_t * ts_block.get(j - 1).get(0));
        value_delta_i =
            ts_block.get(alpha).get(1) - (theta0_v + theta1_v * ts_block.get(j - 1).get(1));
      } else if (i == alpha + 1) {
        timestamp_delta_i =
            ts_block.get(alpha + 1).get(0) - (theta0_t + theta1_t * ts_block.get(alpha - 1).get(0));
        value_delta_i =
  ...
  private static ArrayList<Integer> adjustn0(
    ...
    int value_delta_i;
    timestamp_delta_i =
        ts_block.get(0).get(0) - (theta0_t + theta1_t * ts_block.get(block_size - 1).get(0));
    value_delta_i =
        ts_block.get(0).get(1) - (theta0_v + theta1_v * ts_block.get(block_size - 1).get(1));
    if (timestamp_delta_i > timestamp_delta_max) {
      timestamp_delta_max = timestamp_delta_i;
    }
    if (timestamp_delta_i < timestamp_delta_min) {
  ...
  //                             ArrayList<Integer> raw_length, int index, ArrayList<Integer>
  // theta){
  ...
  //
  //    if(epsilon_r_i_star_plus_1 > raw_length.get(1) || epsilon_v_i_star_plus_1 >
  // raw_length.get(2))
  //      return 0;
  //
  //
  //    for(int j = 1;j<block_size;j++){
  //      if(j!=i_star){
  //        int epsilon_r_j = (int) ((double) ts_block.get(j).get(0) - theta0_r -theta1_r * (double)
  // ts_block.get(i_star).get(0));
  //        int epsilon_v_j = (int) ((double) ts_block.get(j).get(1) - theta0_v - theta1_v *
  // (double) ts_block.get(i_star).get(1));
  //        int epsilon_r_i_star = (int) ((double) ts_block.get(i_star).get(0) - theta0_r -theta1_r
  // * (double) ts_block.get(j-1).get(0));
  //        int epsilon_v_i_star = (int) ((double) ts_block.get(i_star).get(1) - theta0_r -theta1_r
  // * (double) ts_block.get(j-1).get(1));
  ...
  public static int getIStar(
    ...
    if (index == 0) {
      for (int j = 1; j < block_size; j++) {
        int epsilon_v_j =
            ts_block.get(j).get(1) - (theta0_v + theta1_v * ts_block.get(j - 1).get(1));
      ...
      for (int j = 1; j < block_size; j++) {
        int epsilon_r_j =
            ts_block.get(j).get(0) - (theta0_t + theta1_t * ts_block.get(j - 1).get(0));
  ...
  //
  //    for(int j = 1;j<block_size;j++){
  //      int epsilon_r_j = getBitWith((int) ((double) ts_block.get(j).get(0) - theta0_r -theta1_r *
  // (double) ts_block.get(j-1).get(0)));
  //      int epsilon_v_j = getBitWith ((int) ((double) ts_block.get(j).get(1) - theta0_v - theta1_v
  // * (double) ts_block.get(j-1).get(1)));
  //      if(index == 1){
  //        if(epsilon_v_j<=raw_length.get(2) && epsilon_r_j < i_star_bit_width && epsilon_r_j <
  // raw_length.get(1)){
  //          i_star_bit_width = epsilon_r_j;
  //          i_star = j;
  //        }
  //      }else{
  //        if(epsilon_v_j<raw_length.get(2) && epsilon_v_j < i_star_bit_width && epsilon_r_j <=
  // raw_length.get(1)){
  ...
  public static ArrayList<Byte> ReorderingRegressionEncoder(
      ArrayList<ArrayList<Integer>> data, int block_size, int td, int rate) {
    block_size++;
  ...
  //      ArrayList<Integer> i_star_ready_reorder = new ArrayList<>();
  //      ArrayList<Integer> theta_reorder = new ArrayList<>();
  //      ArrayList<ArrayList<Integer>> ts_block_delta_reorder = getEncodeBitsRegression( ts_block,
  // block_size, reorder_length,
  ...
  // encode2Bytes(ts_block_delta,raw_length,theta,result2);
  //        encoded_result.addAll(cur_encoded_result);
  //
  //      }
  //      else{
  ...
  //                i_star_ready_reorder,theta_reorder);
  //        ArrayList<Byte> cur_encoded_result =
  // encode2Bytes(ts_block_delta_reorder,reorder_length,theta_reorder, result2);
  //        encoded_result.addAll(cur_encoded_result);
  //      }
  //    }
  ////    System.out.println(count_raw);
        ...
        for (File f : tempList) {
          ...
          for (int i = 0; i < repeatTime; i++) {
            ...
            ArrayList<Byte> buffer =
                ReorderingRegressionEncoder(data, 256, dataset_map_td.get(file_i), learning_rate);
