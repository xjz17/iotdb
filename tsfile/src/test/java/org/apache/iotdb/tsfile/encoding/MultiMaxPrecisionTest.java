...
public class MultiMaxPrecisionTest {
  ...
  public static int getBitWith(long num) {
    if (num == 0) return 1;
    else return 64 - Long.numberOfLeadingZeros(num);
  }
    ...
    for (int file_i = 0; file_i < input_path_list.size(); file_i++) {
      ...
      for (File f : tempList) {
        ...
        for (int i = 0; i < repeatTime; i++) {
          ...
          //                    double ratioTmp = (double) encode_elem_length / (double)
  ...
  private static ArrayList<Byte> MultiMaxPrecisionTest(
      ArrayList<Double> data, int block_size, int max_precision) throws IOException {
  ...
  private static int MultiMaxPrecisionTest2(
      ArrayList<Double> data, int block_size, int max_precision) throws IOException {
    int length_all = data.size();
    int encoded_length_all = 0;
    byte[] length_all_bytes = int2Bytes(length_all);
