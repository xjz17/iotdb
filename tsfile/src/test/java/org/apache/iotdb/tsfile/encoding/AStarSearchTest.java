...
//    ArrayList<ArrayList<Integer>> ts_block_bit_width = new ArrayList<>();
  ...
  public static ArrayList<ArrayList<ArrayList<Integer>>> permute(ArrayList<ArrayList<Integer>> nums) {
    ...
    permuteHelperBitWidth(nums, used, current, result);
  ...
  public static ArrayList<Integer> getMaxBitWidthForPrune(ArrayList<ArrayList<Integer>> ts_block){
    ...
    int timestamp_delta_min = Integer.MAX_VALUE;
    int value_delta_min = Integer.MAX_VALUE;
    ArrayList<Integer> max_bit_width = new ArrayList<>();
    ArrayList<ArrayList<Integer>> ts_block_delta = new ArrayList<>();
    ...
    int max_bit_width_interval = getBitWith(max_interval);
    int max_bit_width_value = getBitWith(max_value);
    max_bit_width.add(max_bit_width_interval);
    max_bit_width.add(max_bit_width_value);

    return max_bit_width;

  }

  private static void permuteHelperBitWidth(ArrayList<ArrayList<Integer>> nums, boolean[] used, ArrayList<ArrayList<Integer>> current, ArrayList<ArrayList<ArrayList<Integer>>> result) {
    if (current.size() == nums.size()) {
//      if(result.size()==1){
//        ArrayList<Integer> b0 = getMaxBitWidthForPrune(current);
//        ArrayList<Integer> b1 = getMaxBitWidthForPrune(result.get(0));
//        if(b0.get(0)+b0.get(1)<b1.get(0)+b1.get(1)){
//          System.out.println(current);
//          result.set(0,new ArrayList<>(current));
//        }
//      }else
    ...
    int i_cur = 0;
    ArrayList<Integer> i_cur_list = new ArrayList<>();
...
//        System.out.println("b0.get(0)+b0.get(1)+b1.get(0)+b1.get(1)="+(b0.get(0)+b0.get(1)+b1.get(0)+b1.get(1)));
//        System.out.println("b0.get(0)+b0.get(1)="+(b0.get(0)+b0.get(1)));
//        System.out.println("b1.get(0)="+(b1.get(0)));
//        System.out.println("b1.get(1)="+(b1.get(1)));
        fn = b0.get(0)+b0.get(1);
      }
//      else if(b0.get(0)+b0.get(1) == fn){
//        i_cur_list.add(i);
//      }
      used[i] = false;
//      used[i] = true;
...
//      current.remove(current.size() - 1);
//    }
//    System.out.println("i_cur_list="+i_cur_list);
    count ++;
  }

  private static void permuteHelper1(ArrayList<ArrayList<Integer>> nums, boolean[] used, ArrayList<ArrayList<Integer>> current, ArrayList<ArrayList<ArrayList<Integer>>> result) {
    if (current.size() == nums.size()) {
      if(result.size()==1){
        ArrayList<Integer> b0 = getSumAbosulte(current);
        ArrayList<Integer> b1 = getSumAbosulte(result.get(0));
        if(b0.get(0)+b0.get(1)<b1.get(0)+b1.get(1)){
          System.out.println(current);
          result.set(0,new ArrayList<>(current));
        }
      }else result.add(new ArrayList<>(current));
      return;
    }
    int fn = Integer.MAX_VALUE;
    ArrayList<Integer> i_cur_list = new ArrayList<>();
...
//          System.out.println("used[j]="+used[j]);
//          System.out.println("nums.get(j)="+nums.get(j));
//        }
        if (used[j]) {
          continue;
        }
...
//        System.out.println("b1.get(1)="+(b1.get(1)));
      ...
      }else if(b0.get(0)+b0.get(1)+b1.get(0)+b1.get(1) == fn){
        i_cur_list.add(i);
      }
      used[i] = false;
//      used[i] = true;
...
//      ArrayList<Integer> bitwidth_time = getSumBitWidth(ts_block);
      if (i==0){
        printTSBlock(ts_block,"C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_bit_width\\"+dataset_name+"0.csv");
        quickSort(ts_block_reorder,1,0,block_size-1);

        printTSBlock(ts_block_reorder,"C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_bit_width\\"+dataset_name+"1.csv");
      }
//      for(int alpha=1;alpha < 10;alpha++){
      if (i==0)
        printTSBlock(resultOptimal,"C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_bit_width\\"+dataset_name+"2.csv");
