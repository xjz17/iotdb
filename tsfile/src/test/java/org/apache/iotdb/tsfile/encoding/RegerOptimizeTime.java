...
public class RegerOptimizeTime {
  ...
  public static void countingSort(
      ArrayList<ArrayList<Integer>> ts_block, int max, int min, int index) {
    int n = ts_block.size();
  ...
  public static void combinationSort(
      ArrayList<ArrayList<Integer>> ts_block, int low, int high, int index, int memory_limit) {
    ...
    if (max - min > memory_limit) quickSort(ts_block, index, low, high - 1);
    else countingSort(ts_block, max, min, index);
  }
    ...
    for (int i = 0; i < block_num; i++) {
      ...
      ArrayList<ArrayList<Integer>> ts_block_delta_reorder =
          getEncodeBitsRegression(
              ts_block_reorder, block_size, reorder_length, i_star_ready_reorder, theta_reorder);
  ...
  public static void main(@org.jetbrains.annotations.NotNull String[] args) throws IOException {
    ...
    String parent_dir =
        "C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\reorder\\result_evaluation\\OptimizeTime";
