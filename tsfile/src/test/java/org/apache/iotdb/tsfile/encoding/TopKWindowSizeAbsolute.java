...
//    for (byte b: td_common_byte) encoded_result.add(b);
  ...
  public static long getEncodeDeltaTsBlock(ArrayList<ArrayList<Integer>> ts_block_delta,ArrayList<Integer> result){
    ArrayList<Byte> encoded_result = new ArrayList<>();
...
//    byte[] td_common_byte = int2Bytes(result2.get(0));
//    for (byte b: td_common_byte) encoded_result.add(b);
    getEncodeDeltaTsBlockBits += ((long) (getBitWith(max_interval) + getBitWith(max_value)) *ts_block_delta.size());
    return getEncodeDeltaTsBlockBits;
  }

  public static int getDeltaTsBlockTime(ArrayList<ArrayList<Integer>> ts_block_delta, ArrayList<Integer> result){
    int max_interval = Integer.MIN_VALUE;
    int max_value = Integer.MIN_VALUE;
    int block_size = ts_block_delta.size();
//    System.out.println(ts_block_delta);
...
//    System.out.println("value_delta_max"+value_delta_max);
  ...
  public static long getTopKBits(ArrayList<ArrayList<Integer>> ts_block, ArrayList<Integer> raw_length, double threshold){
...
//    System.out.println("encoded bits:"+(outlier_bits + cur_encoded_result.size() * 8L));
  ...
  public static long ReorderingDeltaEncoder(ArrayList<ArrayList<Integer>> data,int block_size,double threshold,String dataset) throws IOException {
    block_size ++;
...
//      ArrayList<Integer> i_star_ready = new ArrayList<>();
      ...
      for(int w=bit_width_time+4;w<bit_width_time+5;w++){
        int interval_time = (int) (power(2,w) -1);
...
//          System.out.println(alpha);
      ...
      if (i==0){
        printTSBlock(ts_block,"C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_top_k\\"+dataset+"0.csv");
        quickSort(ts_block_reorder,1,0,ts_block_reorder.size()-1);
        printTSBlock(ts_block_reorder,"C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_top_k\\"+dataset+"1.csv");
      }
...
//      System.out.println(reorder);
      ...
      if (i==0)
        printTSBlock(ts_block_reordering,"C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_top_k\\"+dataset+"2.csv");
...
//      bits_encoded_data += Math.min(outlier_bits + cur_encoded_result.size() * 8L, encoded_bits_order_by_time);
...
//    dataset_block_size.add(1024);
    ...
    String output =  "C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\reorder\\result_evaluation\\compression_ratio\\top_k_1nn_ws\\";
