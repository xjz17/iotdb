...
public class LearnTopKRegerDeltaDecode {
    ...
    private static byte[] stringBuilderToBytes(StringBuilder sb) {
        int numBytes = (int) Math.ceil((double) sb.length() / 8);
        byte[] bytes = new byte[numBytes];

        for (int i = 0; i < numBytes; i++) {
            int endIndex = Math.min((i + 1) * 8, sb.length());
            String binaryString = sb.substring(i * 8, endIndex);
            bytes[i] = (byte) Integer.parseInt(binaryString, 2);
        }
...
//          getEncodeBitsRegression(ts_block, block_size, raw_length, i_star_ready_reorder);
      ArrayList<Byte> cur_encoded_result = learnKDeltaGetByte(ts_block, result2);
...
//        ArrayList<ArrayList<Integer>> ts_block_order = new ArrayList<>();
        ...
        int min_bits = 0;
        int final_is_split_or_merge_outlier = 0;
        int final_is_bitmap = 0;
        ...
        ArrayList<Integer> final_bitmap = new ArrayList<>();
            ...
            for (int k_spread_time : spread_time) {
                ...
                for (int k_start_value : start_value) {
                    for (int k_spread_value : spread_value) {
                        ...
                        int is_split_or_merge_outlier = SPLIT;
                        int is_bitmap = BITMAP;
                        int cur_bits_split = getBitwidthOutlierFour(outlier_top_k_left_time,outlier_top_k_right_time,
                                outlier_top_k_left_value,outlier_top_k_right_value,block_size);
                        int cur_bits_merge = getBitwidthOutlier(outlier_top_k, block_size);
                        int cur_bits = cur_bits_split;
                        if(cur_bits_split > cur_bits_merge){
                            cur_bits = cur_bits_merge;
                            is_split_or_merge_outlier = MERGE;
                        }

                        if(block_size > outlier_size * getBitWith(block_size)){
                            is_bitmap = INDEX;
                        }
...
//                            System.out.println(min_bits);
                            ...
                            final_is_split_or_merge_outlier = is_split_or_merge_outlier;
                            final_is_bitmap = is_bitmap;
...
//        System.out.println(final_outlier_top_k_index);
    ...
    private static ArrayList<Byte> learnKDeltaGetByte(ArrayList<ArrayList<Integer>> ts_block, ArrayList<Integer> result) {
        int timestamp_delta_min = Integer.MAX_VALUE;
        int value_delta_min = Integer.MAX_VALUE;
        int block_size = ts_block.size();
...
//        ArrayList<ArrayList<Integer>> ts_block_order = new ArrayList<>();
        ...
        ArrayList<Integer> final_outlier_top_k_index = new ArrayList<>();
        ArrayList<Integer> final_bitmap = new ArrayList<>();
        ArrayList<Integer> final_split_outlier_bitmap = new ArrayList<>();
        ArrayList<ArrayList<Integer>> outlier_top_k_left_time = new ArrayList<>();
        ArrayList<ArrayList<Integer>> outlier_top_k_right_time = new ArrayList<>();
        ArrayList<ArrayList<Integer>> outlier_top_k_left_value = new ArrayList<>();
        ArrayList<ArrayList<Integer>> outlier_top_k_right_value = new ArrayList<>();
            ...
            for (int k_spread_time : spread_time) {
                ...
                for (int k_start_value : start_value) {
                    for (int k_spread_value : spread_value) {
                        int k_end_value = k_spread_value + k_start_value;
                        ArrayList<ArrayList<Integer>> outlier_top_k = new ArrayList<>();
                        outlier_top_k_left_time = new ArrayList<>();
                        outlier_top_k_right_time = new ArrayList<>();
                        outlier_top_k_left_value = new ArrayList<>();
                        outlier_top_k_right_value = new ArrayList<>();
                        ArrayList<Integer> split_outlier_bitmap = new ArrayList<>();
                        ...
                        for (int i = 1; i < block_size; i++) {
                            if (ts_block_delta.get(i).get(0) < k_start_time || ts_block_delta.get(i).get(0) > k_end_time
                                ...
                                if (!outlier_top_k_index.contains(i)) {
                                    ...
                                    if(ts_block_delta.get(i).get(0) < k_start_time){
                                        outlier_top_k_left_time.add(tmp);
                                        split_outlier_bitmap.add(0);
                                    } else if (ts_block_delta.get(i).get(0) > k_end_time) {
                                        outlier_top_k_right_time.add(tmp);
                                        split_outlier_bitmap.add(1);
                                    } else if ( ts_block_delta.get(i).get(1) < k_start_value) {
                                        outlier_top_k_left_value.add(tmp);
                                        split_outlier_bitmap.add(2);
                                    } else if (ts_block_delta.get(i).get(1) > k_end_value) {
                                        outlier_top_k_right_value.add(tmp);
                                        split_outlier_bitmap.add(3);
                                    }
                                    outlier_top_k_index.add(i);
                                }
                            } else {
...
//                                new_ts_block_bit_width.add(ts_block_bit_width.get(i));
                        ...
                        cur_bits += (getBitWith(k_spread_time) + getBitWith(k_spread_value)) * (block_size - outlier_size - 1);
                        cur_bits += 64; // 0-th
                        cur_bits += 64; // min_delta
                        if (cur_bits < min_bits) {
                            min_bits = cur_bits;
//                            System.out.println(min_bits);
                            ...
                            if(is_split_or_merge_outlier == SPLIT){
                                final_split_outlier_bitmap = split_outlier_bitmap;
                            }
...
//                            System.out.println(final_outlier_top_k_index);
        ...
        int  k = final_outlier_top_k.size();
        k <<= 1;
        k += final_is_split_or_merge_outlier;
        k += final_is_bitmap;

        byte[] length_all_bytes = int2Bytes(k);
        for (byte b : length_all_bytes) encoded_result.add(b);


        if(final_is_bitmap == INDEX){

            if(final_is_split_or_merge_outlier == SPLIT){
                ArrayList<Integer> tmp = new ArrayList<>();
                for(int j=0;j<4;j++){
                    int i = 0;
                    for(;i<k;i++){
                       if(final_split_outlier_bitmap.get(i)==j){
                           tmp.add(final_outlier_top_k_index.get(i));
                       }
                    }
                }
                final_outlier_top_k_index = new ArrayList<>(tmp);
            }
            int bit_width_block_size = getBitWith(block_size);
            ...
            for(;i<k;i++){
                if(is_eight == 8){
                    is_eight = 0;
                    byte[] bytes = stringBuilderToBytes(sb);
                    for (byte b : bytes) encoded_result.add(b);
                    sb = new StringBuilder();
                }
                int id = final_outlier_top_k_index.get(i);
                String binaryRepresentation = Integer.toBinaryString(id);
                int paddingLength = bit_width_block_size - binaryRepresentation.length();
        ...
        else if (final_is_bitmap == BITMAP) {
            int total_loop = (block_size - 1)/8;
            for(int i =0;i<total_loop;i++){
                int b = 0;
                for(int j=0;j<8;j++){
                    b<<=1;
                    b += final_bitmap.get(i*8+j);
                }
                encoded_result.add((byte) b);
            }
            if(final_is_split_or_merge_outlier == SPLIT){
                int is_four = 0;
                int i = 0;
                ...
                for(;i<k;i++){
                    ...
                    b <<=2;
                    b += final_split_outlier_bitmap.get(i);
                    is_four ++;
                }
                if(is_four != 4){
                    for(;is_four<4;is_four++){
                        b <<=2;
                    }
                    encoded_result.add((byte) b);
                }
...
//        }
        ...
        ArrayList<ArrayList<Integer>> new_ts_block_delta = new ArrayList<>();
        int is_eight = 0;
        for(int i=1;i<block_size;i++){
            if(!final_outlier_top_k_index.contains(i)){
                if(is_eight == 8){
                    is_eight = 0;
                }
                ArrayList<Integer> tmp = new ArrayList<>();
                tmp.add(ts_block_delta.get(i).get(0)-final_k_start_time);
                tmp.add(ts_block_delta.get(i).get(1)-final_k_start_value);
                new_ts_block_delta.add(tmp);
                is_eight ++;
            }
        }
        for(;is_eight<8;is_eight++){
            ArrayList<Integer> tmp = new ArrayList<>();
            tmp.add(0);
            tmp.add(0);
            new_ts_block_delta.add(tmp);
        }
        byte[] bytes = int2Bytes(ts_block.get(0).get(0));
        for (byte b : bytes) encoded_result.add(b);
        bytes = int2Bytes(ts_block.get(0).get(1));
        for (byte b : bytes) encoded_result.add(b);

        bytes = int2Bytes(final_k_start_time);
        for (byte b : bytes) encoded_result.add(b);
        bytes = int2Bytes(final_k_start_value);
        for (byte b : bytes) encoded_result.add(b);


        bytes = bitPacking(new_ts_block_delta,0,getBitWith(final_k_end_time-final_k_start_time));
        for (byte b : bytes) encoded_result.add(b);
        bytes = bitPacking(new_ts_block_delta,1,getBitWith(final_k_end_value-final_k_start_value));
        for (byte b : bytes) encoded_result.add(b);


        if(final_is_split_or_merge_outlier == MERGE){
            if(k==1||k==2||k==3){
                for(ArrayList<Integer> tmp: final_outlier_top_k){
                    bytes = int2Bytes(tmp.get(0));
                    for (byte b : bytes) encoded_result.add(b);
                    bytes = int2Bytes(tmp.get(1));
                    for (byte b : bytes) encoded_result.add(b);
                }
            }else {

            }
            ArrayList<ArrayList<Integer>> ts_block_delta_outlier = getAbsDeltaTsBlock(final_outlier_top_k);
...
//                        buffer_bits = ReorderingRegressionEncoder(data, dataset_block_size.get(file_i), dataset_name.get(file_i));
              buffer = ReorderingRegressionEncoder(data, dataset_block_size.get(file_i), dataset_name.get(file_i));
                    ...
                    double ratioTmp = (double) buffer_bits / (double) (data.size() * Integer.BYTES * 2);
