...
public class RegerDeltaReorderTopKTest {
...
//  }
    ...
    public static int getIStarThreshold(ArrayList<ArrayList<Integer>> ts_block, int block_size, double threshold) {
        ...
        for (int index = 0; index < 2; index++) {
            ArrayList<ArrayList<Integer>> epsilon_v_j_list = new ArrayList<>();
            int epsilon_v_j = ts_block.get(1).get(index) - ts_block.get(0).get(index);
            ArrayList<Integer> tmp = new ArrayList<>();
            tmp.add(1);
            tmp.add(epsilon_v_j);
            epsilon_v_j_list.add(tmp);
            for (int j = 2; j < block_size; j++) {
                epsilon_v_j = ts_block.get(j).get(index) - ts_block.get(j - 1).get(index);
                tmp = new ArrayList<>();
                tmp.add(j);
                tmp.add(epsilon_v_j);
                // 寻找插入位置
            ...
            int i_star_index = (int) ((double) epsilon_v_j_list.size() * threshold);
            if (epsilon_v_j_list.get(i_star_index).get(1) > value_delta_max) {
                i_star = epsilon_v_j_list.get(i_star_index).get(0);
                value_delta_max = epsilon_v_j_list.get(i_star_index).get(1);
            }
        }
...
//            for (int i = 0; i < block_num; i++) {
      ...
      if (i == 0) {
          printTSBlock(
                  ts_block,
                  "C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\series\\delta_reorder\\"
                          + dataset_name
                          + "0.csv");
...
//            System.out.println("threshold " + threshold + ", current bits: " + cur_bits);
...
//                        System.out.println(buffer_bits);
//                        System.out.println((double) (data.size() * Integer.BYTES * 2));
