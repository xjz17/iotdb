...
public class ShowSequenceAlphaWholeAbsoluteDeltaTest {
  ...
  private static ArrayList<Integer> adjustTo0DeltaWhole(
      ArrayList<ArrayList<Integer>> ts_block, int alpha) {
    int block_size = ts_block.size();
    assert alpha != block_size - 1;
    assert alpha != 0;
    ...
    raw_bit_width_timestamp_sum +=
        abs(ts_block.get(alpha + 1).get(0) - ts_block.get(alpha - 1).get(0));
  ...
  private static ArrayList<Integer> adjustAlphaToJDeltaWhole(
      ArrayList<ArrayList<Integer>> ts_block, int alpha, int j) {
    ...
    delta_raw_bit_width_timestamp_sum +=
        abs(ts_block.get(alpha + 1).get(0) - ts_block.get(alpha - 1).get(0));
    delta_raw_bit_width_value_sum +=
        abs(ts_block.get(alpha + 1).get(1) - ts_block.get(alpha - 1).get(1));

    //    System.out.println("delta_raw_bit_width_value_sum = "+delta_raw_bit_width_value_sum);
    //    System.out.println("j = "+j);
    //    System.out.println("alpha = "+alpha);
    //    System.out.println("0) abs(ts_block.get(j).get(1) -   ts_block.get(alpha).get(1)) =
    // "+(abs(ts_block.get(j).get(1) -   ts_block.get(alpha).get(1))));
    //    System.out.println("0) abs(ts_block.get(alpha).get(1) - ts_block.get(j-1).get(1)) =
    // "+(abs(ts_block.get(alpha).get(1) - ts_block.get(j-1).get(1))));
    //    System.out.println("0) abs(ts_block.get(alpha+1).get(1) - ts_block.get(alpha-1).get(1)) =
    // "+(abs(ts_block.get(alpha+1).get(1) - ts_block.get(alpha-1).get(1))));
    ...
    delta_raw_bit_width_timestamp_sum -=
        abs(ts_block.get(alpha + 1).get(0) - ts_block.get(alpha).get(0));
    delta_raw_bit_width_value_sum -=
        abs(ts_block.get(alpha + 1).get(1) - ts_block.get(alpha).get(1));

    //    System.out.println("delta_raw_bit_width_value_sum = "+delta_raw_bit_width_value_sum);
    //    System.out.println("j = "+j);
    //    System.out.println("alpha = "+alpha);
    //    System.out.println("1) abs(ts_block.get(j).get(1) -   ts_block.get(j-1).get(1)) =
    // "+(abs(ts_block.get(j).get(1) -   ts_block.get(j-1).get(1))));
    //    System.out.println("1) abs(ts_block.get(alpha).get(1) -   ts_block.get(alpha-1).get(1)) =
    // "+(abs(ts_block.get(alpha).get(1) -   ts_block.get(alpha-1).get(1))));
    //    System.out.println("1) abs(ts_block.get(alpha+1).get(1) - ts_block.get(alpha).get(1)) =
    // "+(abs(ts_block.get(alpha+1).get(1) - ts_block.get(alpha).get(1))));
  ...
  private static ArrayList<Integer> adjustNWhole(ArrayList<ArrayList<Integer>> ts_block, int j) {
    ...
    int delta_raw_bit_width_value_sum = 0;
    delta_raw_bit_width_timestamp_sum +=
        abs(ts_block.get(j).get(0) - ts_block.get(block_size - 1).get(0));
    delta_raw_bit_width_value_sum +=
        abs(ts_block.get(j).get(1) - ts_block.get(block_size - 1).get(1));

    delta_raw_bit_width_timestamp_sum +=
        abs(ts_block.get(block_size - 1).get(0) - ts_block.get(j - 1).get(0));
    delta_raw_bit_width_value_sum +=
        abs(ts_block.get(block_size - 1).get(1) - ts_block.get(j - 1).get(1));

    delta_raw_bit_width_timestamp_sum -= abs(ts_block.get(j).get(0) - ts_block.get(j - 1).get(0));
    delta_raw_bit_width_value_sum -= abs(ts_block.get(j).get(1) - ts_block.get(j - 1).get(1));

    delta_raw_bit_width_timestamp_sum -=
        abs(ts_block.get(block_size - 1).get(0) - ts_block.get(block_size - 2).get(0));
    delta_raw_bit_width_value_sum -=
        abs(ts_block.get(block_size - 1).get(1) - ts_block.get(block_size - 2).get(1));
  ...
  private static ArrayList<Integer> adjustNTo0Whole(ArrayList<ArrayList<Integer>> ts_block) {
    ...
    int delta_raw_bit_width_value_sum = 0;
    delta_raw_bit_width_timestamp_sum +=
        abs(ts_block.get(0).get(0) - ts_block.get(block_size - 1).get(0));
    delta_raw_bit_width_value_sum +=
        abs(ts_block.get(0).get(1) - ts_block.get(block_size - 1).get(1));
    ...
    for (int i = 0; i < 1; i++) {
      ...
      printTSBlock(
          ...
          "C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_top_k\\"
              ...
              + "1.csv");
      int change_pos = -1;
      //      for(int alpha=318;alpha < 319;alpha++){
      for (int alpha = 1; alpha < block_size - 1; alpha++) {
        ...
        for (int beta = 1; beta < alpha; beta++) {
          ...
          if (b.get(0) == 0 || b.get(1) == 0)
            System.out.println("b.get(0):" + b.get(0) + ",b.get(1):" + b.get(1));
        ...
        if (beta_star != -1) {
          if (change_pos != alpha - beta_star) {
            change_pos = alpha - beta_star;
            System.out.println(
                "alpha:"
                    + alpha
                    + "(Move back "
                    ...
                    + "beta:"
                    + beta_star);
            System.out.println("value:" + ts_block.get(alpha).get(1));
            System.out.println("...");
          }
      ...
      if (beta_star != -1) {
        if (change_pos != block_size - beta_star - 1) {
          change_pos = block_size - beta_star - 1;
          System.out.println(
              "alpha:"
                  + (block_size - 1)
                  + "(Move back "
                  + (block_size - 1 - beta_star)
                  + " positions)"
                  + "->"
                  + "beta:"
                  + beta_star);
          System.out.println("value:" + ts_block.get(block_size - 1).get(1));
          System.out.println("...");
        }
  ...
  public static void main(@org.jetbrains.annotations.NotNull String[] args) throws IOException {
    ...
    for (int file_i = 4; file_i < 5; file_i++) {
      ...
      for (File f : tempList) {
        ...
        inputStream.close();
        for (double threshold = 0; threshold < 0.01; threshold += 0.1) {
          long encodeTime = 0;
          long decodeTime = 0;
          double ratio = 0;
