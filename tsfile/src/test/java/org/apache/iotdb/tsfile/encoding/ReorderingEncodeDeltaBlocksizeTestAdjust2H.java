...
public class ReorderingEncodeDeltaBlocksizeTestAdjust2H {
  ...
  public static void splitTimeStamp(
  ...
  public static ArrayList<ArrayList<Integer>> getEncodeBitsDelta(
  ...
  public static int getIStar(
    ...
    int timestamp_delta_max_value =
        ts_block.get(timestamp_delta_max_index).get(0)
            - ts_block.get(timestamp_delta_max_index - 1).get(0)
            - timestamp_delta_min;
    int value_delta_max_value =
        ts_block.get(value_delta_max_index).get(1)
            - ts_block.get(value_delta_max_index - 1).get(1)
            - value_delta_min;
    if (timestamp_delta_max_value >= value_delta_max_value) i_star = timestamp_delta_max_index;
  ...
  public static ArrayList<Byte> ReorderingDeltaEncoder(
    ...
    for (int i = 0; i < block_num; i++) {
      ...
      ArrayList<ArrayList<Integer>> ts_block_delta =
          getEncodeBitsDelta(ts_block, block_size, raw_length);
      ...
      ArrayList<ArrayList<Integer>> ts_block_delta_reorder =
          getEncodeBitsDelta(ts_block, block_size, reorder_length);
      int i_star;
      int j_star;
    ...
    if (remaining_length != 0) {
      ...
      ArrayList<Integer> raw_length =
          new ArrayList<>(); // length,max_bit_width_interval,max_bit_width_value,max_bit_width_deviation
      ArrayList<ArrayList<Integer>> ts_block_delta =
          getEncodeBitsDelta(ts_block, remaining_length, raw_length);

      // value-order
      quickSort(ts_block, 1, 0, remaining_length - 1);
      ArrayList<Integer> reorder_length = new ArrayList<>();
      ArrayList<ArrayList<Integer>> ts_block_delta_reorder =
          getEncodeBitsDelta(ts_block, remaining_length, reorder_length);
  ...
  public static ArrayList<ArrayList<Integer>> ReorderingDeltaDecoder(
    ...
    while (decode_pos < encoded.size()) {
      ...
      decode_pos += 4;
      time_list =
          decodebitPacking(encoded, decode_pos, max_bit_width_time, min_delta_interval, block_size);
      decode_pos += max_bit_width_time * (block_size - 1) / 8;

      int max_bit_width_value = bytes2Integer(encoded, decode_pos, 4);
      decode_pos += 4;
      value_list =
          decodebitPacking(encoded, decode_pos, max_bit_width_value, min_delta_value, block_size);
  ...
  public static void main(@org.jetbrains.annotations.NotNull String[] args) throws IOException {
    ...
    //            "\\vary_parameter\\rd_ratio\\Metro-Traffic_ratio.csv");
    ...
    //
    // output_path_list.add("C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\reorder\\result_evaluation" +
    //            "\\vary_parameter\\rd_ratio\\Nifty-Stocks_ratio.csv");
    ...
    //
    // output_path_list.add("C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\reorder\\result_evaluation" +
    //            "\\vary_parameter\\rd_ratio\\USGS-Earthquakes_ratio.csv");
        ...
        for (File f : tempList) {
          ...
          for (int i = 0; i < repeatTime; i++) {
            ...
            ArrayList<Byte> buffer =
                ReorderingDeltaEncoder(data, block_size, dataset_map_td.get(file_i));
