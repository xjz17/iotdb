...
public class SubcolumnTest {
  ...
  public static void main(@org.jetbrains.annotations.NotNull String[] args) throws IOException {
    ...
    for (int file_i = 0; file_i < input_path_list.size(); file_i++) {
      ...
      int fileRepeat = 0;
      //            ArrayList<Integer> columnIndexes = new ArrayList<>(); // set the column indexes
      // of compressed
      //            for (int i = 0; i < 2; i++) {
  ...
  private static ArrayList<Byte> SubcolumnEncoder(
      ArrayList<Double> data, int block_size, int max_precision) {
    ...
    for (int i = 0; i < block_num; i++) {
      ...
      for (int j = 1; j < block_size; j++) {
        ...
        //                long intager = (0xffffffffffffffffL << (64 - (long)cur_exp))>>12) ^
        // longBits;
        String intstring;
        ...
        //                binaryString = new StringBuilder(binaryString.substring(11, 11 +
        // max_precision_v + cur_exp));
      ...
      //                encoded_length_all += ((double)Math.log(max_exp-1023) /(double)
      // Math.log(2));
      //            }
      //
      //            else{
      ////                System.out.println((double)Math.log(max_exp-min_exp) /(double)
      // Math.log(2)+1);
      //                encoded_length_all +=( ( (double)Math.log(max_exp-min_exp) /(double)
      // Math.log(2)+1)*block_size);
      //            }
      //            System.out.println(encoded_length_all);
      ...
      if (max_int - min_int != 0)
        ...
        //            else
        encoded_length_all +=
            (((double) Math.log(max_int - min_int) / (double) Math.log(2) + 1 / 8)
                * ((double) block_size));
  ...
  //    private static ArrayList<Byte> SubcolumnEncoder(ArrayList<Double> data, int block_size, int
  // max_precision) {
  //        ArrayList<Byte> encoded_result = new ArrayList<Byte>();
  //        int length_all = data.size();
  //        int encoded_length_all = 0;
  ...
  //                double cur = numberOffset * data.get(j + i * block_size) - numberOffset *
  // data.get(j+i*block_size-1);
  //                if (min_delta > cur) {
  //                    min_delta = cur;
  //                }
  //                ts_block.add(cur);
  ...
  //            ArrayList<Byte> ts_block_encoded = getEncodeBitsSubColumn(ts_block, block_size,
  // max_precision, offset, raw_length);
  ...
  private static ArrayList<Byte> getEncodeBitsSubColumn(
      ArrayList<Double> ts_block,
      int block_size,
      int max_precision,
      int offset,
