...
public class RegerDeltaSumAbsoluteTest {
  ...
  private static ArrayList<Integer> getSumBitWidth(ArrayList<ArrayList<Integer>> ts_block) {
    ...
    for (int i = 1; i < block_size; i++) {
      ...
      raw_bit_width_timestamp_sum += getBitWith(timestamp_delta_i);
      raw_bit_width_value_sum += getBitWith(value_delta_i);
    }
  ...
  private static ArrayList<Integer> getTopkBitWidth(ArrayList<ArrayList<Integer>> ts_block) {
    ...
    for (int i = 1; i < block_size; i++) {
      ...
      tmp = new ArrayList<>();
      int t_bit_width = getBitWith(timestamp_delta_i);
      int v_bit_width = getBitWith(value_delta_i);
      tmp.add(t_bit_width);
      tmp.add(v_bit_width);
      ts_block_delta_bitwidth.add(tmp);
  ...
  public static int getJStar(
    ...
    for (int i = 1; i < block_size; i++) {
      ...
      delta.add(delta_i);
      raw_bit_width_timestamp_sum += getBitWith(delta_t_i);
      raw_bit_width_value_sum += getBitWith(delta_v_i);
    }
    System.out.println(
        Arrays.toString(new int[] {raw_bit_width_timestamp_sum, raw_bit_width_value_sum}));
  ...
  private static ArrayList<Integer> adjustAlphaToJ(
    ...
    //      int delta_t_i = abs(ts_block.get(i).get(0) - ts_block.get(i - 1).get(0);
    //      int delta_v_i = abs(ts_block.get(i).get(1) - ts_block.get(i - 1).get(1);
    //      raw_bit_width_timestamp_sum += getBitWith(delta_t_i-timestamp_delta_min);
    //      raw_bit_width_value_sum += getBitWith(delta_v_i-value_delta_min);
    //    }
    b.add(raw_bit_width_timestamp_sum);
    b.add(raw_bit_width_value_sum);
  ...
  public static ArrayList<Byte> ReorderingRegressionEncoder(
    ...
    for (int i = 0; i < 1; i++) {
      ...
      printTSBlock(
          ts_block,
          "C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_top_k\\0.csv");
      ...
      printTSBlock(
          ts_block,
          "C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_top_k\\1.csv");
      ...
      //        }
      //        ArrayList<Integer> bit_width_length = new ArrayList<>(); //
      // length,max_bit_width_interval,max_bit_width_value,max_bit_width_deviation
      //        i_star_ready = new ArrayList<>();
      //         getEncodeBitsRegression( ts_block_seg,  ts_block_seg.size(), bit_width_length,
      // i_star_ready);
      ...
      if (bitwidth_time.get(0) + bitwidth_time.get(0)
          <= bitwidth_value.get(0) + bitwidth_value.get(1)) {
        quickSort(ts_block, 0, 0, block_size - 1);
        count_raw++;
      ...
      while (j_star != -1 && i_star != -1) {
        ...
        i_star = getIStarAbsolute(ts_block, block_size, raw_length);
        System.out.println("i_star: " + i_star);
      ...
      printTSBlock(
          ts_block,
          "C:\\Users\\xiaoj\\Documents\\GitHub\\encoding-reorder\\vldb\\test_top_k\\2.csv");
      ts_block_delta =
          getEncodeBitsRegression(ts_block, block_size, raw_length, i_star_ready_reorder);
